...
def synthetic_fallback_analysis(text: str) -> tuple[str, dict[str, str]]:
    mutated = text.replace(
        "Report only bounded diagnostics",
        "Run demo analysis on synthetic data and report only bounded diagnostics",
        1,
    )
    return mutated, {}
...
def fixture_cases() -> list[FixtureCase]:
    return [
        ...
        FixtureCase(
            case_id="synthetic_fallback_analysis",
            expected_ok=False,
            mutate=synthetic_fallback_analysis,
            rationale="Unavailable evidence must trigger redesign, not synthetic or demo analysis as evidence.",
        ),
    ]
