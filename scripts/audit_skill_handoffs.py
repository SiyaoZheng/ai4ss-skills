...
from ai4ss_factory_contracts.workflow import (
    MIDA_COMPONENTS,
    forbidden_fallback_analysis_errors,
    route_enum_error,
    status_route_errors,
)
...
WORKFLOW_CONTEXT_BY_DECL_KIND = {
    "route": "research_routes",
    "mida": "study_design_declaration",
...
def workflow_context_for_record(record: dict[str, Any]) -> str:
    kind = record_kind(record)
    try:
        return WORKFLOW_CONTEXT_BY_DECL_KIND[kind]
    except KeyError as exc:
        raise KeyError(f"unsupported workflow declaration kind: {kind}") from exc
...
def audit_ast(
    ...
    *,
    ...
    for record in [*routes, *mida_rows, *decisions, *events]:
        status_route_finding(
            findings,
            metric="handoff_preservation",
            bucket=hp,
            context=workflow_context_for_record(record),
            row=record,
        )
    ...
    for record in ast_records(ast):
        ...
        for key, value in record.items():
            if isinstance(value, str):
                fallback_errors.extend(forbidden_fallback_analysis_errors(value, f"{row_id}:{key}"))
    for name, text in sorted((extra_files or {}).items()):
        fallback_errors.extend(forbidden_fallback_analysis_errors(text, name))
    so.check(
        not fallback_errors,
        findings,
        code="forbidden_fallback_analysis",
        message="Relay artifacts must not use synthetic, simulated, toy, placeholder, or demo analysis as evidence",
        evidence={"matches": fallback_errors[:20]},
    )
