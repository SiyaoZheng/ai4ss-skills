...
这对应研究里的一个基本原则：生产证据的人不能仅凭自我叙述完成质量判定。AI4SS 已经有 methods-reviewer、AutoChecklist CLI、validators、LLM-as-judge evals 等机制；Deli 的贡献是把这种 separation of duties 进一步扩展到运行层。
...
- 写作看似完整但 PDF 不可信：回 AutoChecklist CLI 审查或前置证据 skill
